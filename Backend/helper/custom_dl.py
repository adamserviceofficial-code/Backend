...
class ByteStreamer:
    ...
    async def yield_file(self, file_id: FileId, index: int, offset: int, first_part_cut: int, last_part_cut: int, part_count: int, chunk_size: int) -> Union[str, None]: # type: ignore
        ...
        try:
            ...
            prefetch_count = 4  # Reduced from 20 to prevent TimeoutErrors during high concurrency
            tasks = deque()
            
            # Initialize prefetch queue
            for _ in range(min(prefetch_count, part_count)):
                async def fetch_chunk(off):
                ...
                tasks.append(asyncio.create_task(fetch_chunk(offset)))
                offset += chunk_size
                
            while tasks:
                r = await tasks.popleft()
                ...
                if current_part + len(tasks) < part_count:
                    tasks.append(asyncio.create_task(fetch_chunk(offset)))
                    offset += chunk_size

                # Yield the chunk based on cut positions
                if part_count == 1:
            ...
            LOGGER.error(f"Streaming error for DC {file_id.dc_id}: {e}")
            pass
        except Exception as e:
            LOGGER.error(f"Unexpected streaming error for DC {file_id.dc_id}: {e}")
        finally:
            # Cleanup remaining tasks to prevent "Task exception was never retrieved"
            while tasks:
                t = tasks.popleft()
                t.cancel()
                try:
        ...
        if media_session is None:
            if file_id.dc_id != await client.storage.dc_id():
                ...
                for _ in range(6):
                    ...
                    raise Exception(f"Failed to establish media session for DC {file_id.dc_id}")
            else:
                media_session = Session(
                    client,
                    file_id.dc_id,
